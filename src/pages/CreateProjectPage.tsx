...
function getSaved(key: string, fallback = '') {
  return sessionStorage.getItem(`oncue_proj_${key}`) ?? fallback
}
function save(key: string, val: string) {
  sessionStorage.setItem(`oncue_proj_${key}`, val)
}
...
export default function CreateProjectPage() {
  ...
  function handleChange(key: string, val: string, setter: (v: string) => void) {
  ...
  async function handleNext() {
    ...
    try {
      ...
      // 저장 후 임시 데이터 삭제
      ['name','dateType','date','dateEnd','startTime','endTime','venue','people','budget','overview'].forEach((k) => sessionStorage.removeItem(`oncue_proj_${k}`))
    ...
    <div className="min-h-screen bg-[#F4F6F9]">
      ...
      <div className="max-w-2xl mx-auto px-5 pt-7 pb-10">
        ...
        <div className="mb-4">
          ...
          <input className={inp} placeholder="2026 군포시 철쭉축제" value={name} onChange={(e) => handleChange('name', e.target.value, setName)} />
        </div>
        ...
        <div className="mb-4">
          ...
          {dateType === 'single' ? (
            <div className="relative">
              ...
              <input className={`${inp} pl-9`} type="date" value={date} onChange={(e) => handleChange('date', e.target.value, setDate)} />
            </div>
          ) : (
            <div className="flex items-center gap-2">
              ...
              <div className="relative flex-1">
                <i className="ti ti-calendar absolute left-3 top-1/2 -translate-y-1/2 text-[#A0AEC0] text-[15px]" />
                <input className={`${inp} pl-9`} type="date" value={dateEnd} onChange={(e) => handleChange('dateEnd', e.target.value, setDateEnd)} />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input className={`${inp} pl-9`} type="time" value={startTime} onChange={(e) => handleChange('startTime', e.target.value, setStartTime)} />
            </div>
            <span className="text-[#A0AEC0] font-medium">~</span>
            <div className="relative flex-1">
              <i className="ti ti-clock absolute left-3 top-1/2 -translate-y-1/2 text-[#A0AEC0] text-[15px]" />
              <input className={`${inp} pl-9`} type="time" value={endTime} onChange={(e) => handleChange('endTime', e.target.value, setEndTime)} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            ...
            <div className="relative">
              ...
              <input className={`${inp} pl-9`} placeholder="학교 중앙 광장" value={venue} onChange={(e) => handleChange('venue', e.target.value, setVenue)} />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input className={`${inp} pl-9`} placeholder="500명" value={people} onChange={(e) => handleChange('people', e.target.value, setPeople)} />
            </div>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            <input className={`${inp} pl-9`} placeholder="5,000,000원" value={budget} onChange={(e) => handleChange('budget', e.target.value, setBudget)} />
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <textarea className={`${inp} h-[80px] resize-none`} placeholder="행사 목적, 주의사항 등 자유롭게..." value={overview} onChange={(e) => handleChange('overview', e.target.value, setOverview)} />
        </div>
