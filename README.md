...
| 8 | `/my` | 마이페이지 (내 정보 + 내 템플릿 보관함) | ✅ 완성 |
| 9 | `/blog/write` | 블로그 글쓰기 (템플릿 첨부 가능) | ✅ 완성 |
| 10 | `/onboarding/field` | 분야 선택 | ✅ 완성 |
| 11 | `/onboarding/create` | 프로젝트 생성 | ✅ 완성 |
| 12 | `/onboarding/parts/:id` | 파트 구성 & 초대 | ✅ 완성 |
| 13 | `/p/:id/home` | 프로젝트 홈 (템플릿 내보내기/가져오기 + 시뮬레이션) | ✅ 완성 |
| 14 | `/p/:id/my-part` | 담당자 작업 화면 (큐시트 + 체크리스트) | ✅ 완성 |
| 15 | `/p/:id/dashboard` | 본부 대시보드 | ✅ 완성 |
| 16 | `/p/:id/timeline` | 통합 타임라인 | ✅ 완성 |
| 17 | `/p/:id/comms` | 소통 허브 | ✅ 완성 |
| 18 | `/p/:id/live` | 당일 실시간 운영 | ✅ 완성 |
| 19 | `/p/:id/ptt` | 🎙️ 무전 (푸시투토크) | ✅ 완성 |
| 20 | `/p/:id/admin` | 프로젝트 관리자 | ✅ 완성 |
| 21 | `/join` | 초대 링크 참여 페이지 (파트 자동 배정) | ✅ 완성 |

### 관리자 (Firebase DB admins 등록 필요)

| # | 경로 | 화면 | 상태 |
|---|------|------|------|
| 22 | `/admin` | 사이트 전체 관리자 콘솔 | ✅ 완성 |
...
로그인 후 접근 가능한 개인 설정 및 템플릿 보관함 페이지입니다.

### 내 정보 탭
- 닉네임(displayName) 수정
- 전화번호 저장 (`users/{uid}/phone`)
- 비밀번호 변경 (이메일 로그인 전용, 재인증 포함)
- 플랜 현황 — Free / Pro, Pro 만료일 또는 영구 Pro 표시
- 내 프로젝트 통계 — 진행 중 / 완료 / 기획 중 요약
- 회원 탈퇴

### 내 템플릿 탭
- 저장된 `.thanq` 템플릿 목록 조회 (`userTemplates/{uid}/{id}`)
- 다운로드 / 프로젝트에 바로 적용 / 삭제
- 외부 `.thanq` 파일 가져오기 (파일 업로드)

---

## 시뮬레이션 기능 (프로젝트 홈 내)

행사 전 큐시트 충돌·지연을 미리 점검하는 기능입니다. Firebase에 저장되지 않으며 프로젝트 홈 우측 버튼으로 실행합니다.

- **충돌 감지** — 두 파트 이상이 같은 시간대에 겹칠 경우 자동 표시
- **심각도 분류** — `conflict` (심각) / `warning` (주의) / `info` (참고) 3단계
- **전체 타임라인 뷰** — 모든 파트 큐시트를 시간순 통합 뷰로 시각화
- **통계** — 전체 큐 수, 총 소요 시간, 충돌/경고 건수 요약

---

## 초대 링크 참여 (`/join`)

파트별 전용 초대 링크(QR/URL)로 팀원이 클릭 한 번에 프로젝트에 참여하는 기능입니다.

```
/join?code=ABCD12&partKey=abc123&role=member&partName=음향팀
```

- **비로그인 상태**에서 클릭 시 → 세션에 링크 정보 보관 후 로그인 페이지 이동, 로그인 완료 시 자동 접속
- `partKeyMap/{projectId}/{partKey}` 경로로 파트 자동 배정
- 파트 지정 없이 프로젝트 코드만 있을 경우 일반 멤버로 참여
...
[프로젝트 홈] → "템플릿 가져오기" → 파트 + 큐시트 + 체크리스트 자동 세팅
      ↓
[마이페이지] → "내 템플릿 보관함"에서 관리 (userTemplates/{uid}/{id})
```
...
```json
{
  ...
  "passwordHash": "SHA-256 해시 (선택)",
  "allowedEmail": "허용 이메일 (선택)"
}
```
...
├── userTemplates/{uid}/{id}              # 마이페이지 내 템플릿 보관함
└── partKeyMap/{projectId}/{partKey}      # 파트별 초대 링크 키 맵핑
```
...
| # | 규칙 |
|---|------|
| 1 | 모든 데이터는 Firebase Realtime DB에 실시간 저장 — sessionStorage 사용 금지 (단, 초대 링크 리다이렉트용 임시값 예외) |
...
│   ├── simulation/   # 시뮬레이션 모달 (SimulationModal)
